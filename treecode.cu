...
template<template<size_t, typename, TraverseMode ...> class ElemType, template<size_t, typename, TraverseMode ...> class ElemTypeArray, size_t DIM, typename Float, TraverseMode ... ModeOption>
__device__ void initStack(ElemTypeArray<DIM, Float, ModeOption ...> level, size_t levelCt, ElemTypeArray<DIM, Float, ModeOption ...> stack, size_t* stackCt){
	if(threadIdx.x < levelCt){
		...
		ElemType<DIM, Float, ModeOption ...> eHere;
...
template<template<size_t, typename, TraverseMode ...> class ElemType, template<size_t, typename, TraverseMode ...> class ElemTypeArray, size_t DIM, typename Float, TraverseMode ... ModeOption>
__device__ void pushAll(const ElemTypeArray<DIM, Float, ModeOption ...> src, const size_t srcCt, ElemTypeArray<DIM, Float, ModeOption ...> stack, size_t* stackCt){
...
template<size_t DIM, typename Float, TraverseMode Mode> __device__ InteractionType<DIM, Float, Mode> calc_interaction(const PointMass<DIM, Float> &m1, const InteracterType<DIM, Float, Mode> &m2, Float softening){
	InteractionType<DIM, Float, Mode> interaction = freshInteraction();
	bool isParticle = m2.m != 0;
	switch(Mode){
	case Forces:
	...
	case HashInteractions:
		interaction.x[isParticle] = m2.pos[0] ^ (!isParticle) * (m2.pos[1] ^ m2.pos[2]);
		break;
	}
	return interaction;
}
...
#define BUF_MUL 128
	...
	__shared__ InteractionElemType<Float, Mode> pointMass[BUF_MUL*INTERACTION_THRESHOLD];
	__shared__ InteractionElemType<Float, Mode> pointPos[DIM * BUF_MUL*INTERACTION_THRESHOLD];
	__shared__ InteractionElemType<Float, Mode> interactionBuf[TPB * InteractionElems(Mode, DIM, 2)];
	...
	for(size_t j = 0; j < InteractionElems(Mode, DIM, 3); j++){
	...
	for(size_t groupOffset = 0; groupOffset + blockIdx.x < nGroups; groupOffset += gridDim.x){
		...
		initStack<InteracterType,InteracterTypeArray, Mode>(dummyP, 0, pGList, pGLCt);
				...
				if(INTERACTION_THRESHOLD > 0){ // Can't diverge, compile-time constant
					...
					for(innerStartOfs = *pGLCt; innerStartOfs >= (ptrdiff_t)INTERACTION_THRESHOLD; innerStartOfs -= threadsPerPart){
						...
						if(toGrab >= 0){
							...
							interaction = interaction + calc_interaction(particle.mass, pHere, softening);
						}
					}
...
(size_t, GroupInfoArray<3, float, 16>, size_t,
		NodeArray<3, float> *, size_t *, size_t, ParticleArray<3, float>, InteractionTypeArray<3, float, Forces>, float, float, size_t);
*/
...
void traverseTreeCUDA(size_t nGroups, GroupInfoArray<DIM, Float, PPG> groupInfo, size_t startDepth,
					  NodeArray<DIM, Float> treeLevels[MAX_LEVELS], size_t treeCounts[MAX_LEVELS], size_t n, ParticleArray<DIM, Float> particles, InteractionTypeArray<DIM, Float, Mode> interactions, Float softening, Float theta, size_t blockCt){
	...
	traverseTreeKernel<DIM, Float, threadCt, PPG, MAX_LEVELS, INTERACTION_THRESHOLD, Mode><<<dimGrid, dimBlock/*, threadCt * sizeof(Float) * InteractionElems(Mode, DIM, 2)*/>>>(nGroups, cuGroupInfo, startDepth, cuTreeLevels, cuTreeCounts, n, cuParticles, cuInteractions, softening, theta, bfsStackCounters, bfsStackBuffers, stackCapacity);
